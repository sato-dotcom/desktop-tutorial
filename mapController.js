...
function updateMapRotation(rawHeading, currentHeading) {
    try {
        ...
        if (!rotator && appState.debugEnabled) {
             updateDebugPanel({
        ...
        if (!appState.headingUp) {
            ...
            if (Math.abs(diff) > HEADING_SPIKE_THRESHOLD && consecutiveSpikes < MAX_CONSECUTIVE_SKIPS) {
                ...
                // targetAngle is not changed, we just don't update lastDrawnMarkerAngle
            } else {
        ...
        console.log(`[DEBUG-RM2] mode=${debugInfo.mode} raw=${debugInfo.raw.toFixed(1)}° current=${debugInfo.current.toFixed(1)}° target=${debugInfo.target.toFixed(1)}° last=${(debugInfo.last||0).toFixed(1)}° diff=${(debugInfo.diff||0).toFixed(1)}°`);
